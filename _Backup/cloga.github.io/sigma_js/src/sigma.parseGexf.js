...
sigma.publicPrototype.parseGexf = function(gexfPath) {
	...
	var gexf;
	 if($.browser.msie){		
		...
		xmlDoc.async="false";
		xmlDoc.load(gexfPath);
		gexf = xmlDoc;
	 }else{
		 var gexfhttp = window.XMLHttpRequest ?	new XMLHttpRequest() :	new ActiveXObject('Microsoft.XMLHTTP');
	...
	$("#note").hide();
		...
		for(i=0; i<nodesNodes.length; i++){
			...
			for(j=0; j<nodeNodes.length; j++){
				...
				var color;
				var sizeNodes = nodeNode.getElementsByTagName('viz:size');
				...
				var positionNodes = nodeNode.getElementsByTagName('viz:position');
				...
				var colorNodes = nodeNode.getElementsByTagName('viz:color');
				...
				// Create Node
				var node = {
					label:label, size:size, x:x, y:y, attributes:[], color:color};  // The graph node
